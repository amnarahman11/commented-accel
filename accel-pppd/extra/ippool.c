...
static void parse_gw_ip_address(const char *val)
{
	...
	conf_gw_ip_address = inet_addr(val);		// Here we are saving Gateway IP address. In later edits, we have to see if it lies in IP ranges
}
...
static void add_range(struct ippool_t *p, struct list_head *list, const char *name, void (*generate)(struct ippool_t *))
{
	...
	for (i = startip; i <= endip; i++) {
		ip = malloc(sizeof(*ip));		// It looks like iterating all the IPs. We can check if any of them = GW add
		ip->addr = htonl(i);
