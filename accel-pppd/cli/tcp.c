...
/**
This file gets user's command. The first module which receives the command from user over a tcp interface and then forwards the command to other modules
**/
...
struct tcp_client_t {
	...
	uint8_t *cmdline;		// Important: Here the command is being saved and updated
	int xmit_pos;
	int recv_pos;			// Important: Most likely working as a length indicator for current command
	int auth:1;
	int disconnect:1;
};
...
// Important function which gets user's command and forwards that to cli.c for processing
